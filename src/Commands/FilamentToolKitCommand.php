...
class FilamentToolKitCommand extends Command
{
    ...
    public $signature = 'tool-kit:resource{model? : The model name} {--F|force} {--model-namespace=} {--panel=}';

    public $description = 'generate a new Tool Kit resource';

    public function handle(): int
    {
        $model = str($this->argument('model') ?? text(
        ...
        $tableSchemaPath = "{$tablesPagesDirectory}/{$modelClass}TableSchema.php";

        $this->copyStubToApp('TableSchema', $tableSchemaPath, [
            'namespace' => "{$namespace}\\{$resourceClass}\\Tables",
            'modelClass' => $modelClass,
        ]);
